...
class AwisFile {
    ...
    async download() {
        if (!this.domainName || this.exists)
            return;
        try {
    ...
    async fetch() {
        ...
        return new Promise((resolve, reject) => {
            client({
                Action: "UrlInfo",
                Url: domainName,
                ResponseGroup: "Related,TrafficData,ContentData,LinksInCount,RankByCountry"
            }, function (err, data) {
                //console.log(data)
                if (err) {
                    console.error(err);
...
class AwisImporter extends MeasurementsCrawler_1.MeasurementsCrawler {
    writeAllCommand() {
...
exports.AwisImporter = AwisImporter;
