...
const creds = JSON.parse(Disk.read(path.join(__dirname, "ignore", "creds.json")));
const { restKey } = creds;
if (!restKey) {
...
Disk.mkdir(cacheDir);
const falsePositives = new Set(Disk.read(path.join(__dirname, "falsePositives.txt")).split("\n"));
class TrueBaseFileForBooks {
    ...
    async fetch() {
        ...
        if (this.exists)
            return true;
        try {
            await downloadJson(`https://api2.isbndb.com/books/${this.query}?pageSize=1000&column=title`, cacheFilePath);
        }
        catch (err) {
    ...
    get hits() {
        ...
        return this.parsed.books.filter(book => {
            ...
            if (title.toLowerCase().includes("programming"))
                return true;
            if ((_a = book.subjects) === null || _a === void 0 ? void 0 : _a.includes("Computer Science"))
                return true;
...
class ISBNdbImporter extends TrueBaseCrawler_1.TrueBaseCrawler {
...
exports.ISBNdbImporter = ISBNdbImporter;
