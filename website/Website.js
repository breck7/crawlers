...
exports.WebsiteImporter = void 0;
...
class Website {
    ...
    extractGitHub() {
        ...
        if (file.githubRepo)
            return this;
        let potentialHits = Utils.getLinks(this.content.toLowerCase()).filter(link => {
            link = link.trim();
    ...
    extractSocial(name, propName, basePath = "") {
        const { file } = this;
        if (file.has(propName))
            return;
...
class WebsiteImporter extends MeasurementsCrawler_1.MeasurementsCrawler {
    get matches() {
...
exports.WebsiteImporter = WebsiteImporter;
