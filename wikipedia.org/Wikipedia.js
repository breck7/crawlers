...
class WikipediaImporter extends MeasurementsCrawler_1.MeasurementsCrawler {
    async fetchAllCommand() {
        const crawler = new MeasurementsCrawler_1.PoliteCrawler();
        await crawler.fetchAll(this.linkedFiles.map(file => new ConceptFileWithWikipedia(file)));
    }
    ...
    writeToDatabaseCommand() {
        this.linkedFiles.forEach(file => new ConceptFileWithWikipedia(file).writeToDb());
    }
