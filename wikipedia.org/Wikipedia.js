...
class WikipediaImporter extends TreeBaseCrawler_1.TreeBaseCrawler {
    async fetchAllCommand() {
        const crawler = new TreeBaseCrawler_1.PoliteCrawler();
        await crawler.fetchAll(this.linkedFiles.map(file => new TreeBaseFileWithWikipedia(file)));
    }
    ...
    writeToDatabaseCommand() {
        this.linkedFiles.forEach(file => new TreeBaseFileWithWikipedia(file).writeToDb());
    }
    get linkedFiles() {
