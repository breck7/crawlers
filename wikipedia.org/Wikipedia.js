...
exports.WikipediaImporter = void 0;
...
class TrueBaseFileWithWikipedia {
    ...
    getYear(json) {
        ...
        if (!year)
            year = json.info && json.info.yearStarted;
        if (!year)
            year = json.info && json.info.released;
                ...
                if (!years.length) {
                    ...
                    if (!years.length) {
                        while ((matches = yearRegexRelaxed.exec(json.rawInfo))) {
                            ...
                            if (!isNaN(num) && num < 2012)
                                years.push(num);
                        }
                        while ((matches = yearRegexRelaxed.exec(Disk.stripHtml(json.html)))) {
                            num = parseInt(matches[1]);
                            if (!isNaN(num) && num < 2012)
                                years.push(num);
                        }
        ...
        try {
            ...
            try {
                ...
                if (!file.has("creators"))
                    file.set("creators", designerString);
            }
            catch (err) {
...
class WikipediaImporter extends TrueBaseCrawler_1.TrueBaseCrawler {
    async fetchAllCommand() {
        const crawler = new TrueBaseCrawler_1.PoliteCrawler();
        await crawler.fetchAll(this.linkedFiles.map(file => new TrueBaseFileWithWikipedia(file)));
    }
    async updateOneCommand(file) {
        if (!file.has("wikipedia"))
            return;
    ...
    writeToDatabaseCommand() {
        this.linkedFiles.forEach(file => new TrueBaseFileWithWikipedia(file).writeToDb());
    }
...
exports.WikipediaImporter = WikipediaImporter;
