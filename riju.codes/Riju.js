...
class RijuImporter extends MeasurementsCrawler.MeasurementsCrawler {
  ...
  get yamlFiles() {
    return Disk.getFullPaths(cachePath).map(path =>
      (0, yaml_1.parse)(Disk.read(path))
    )
  }
  get matches() {
  ...
  addMissingCommand() {
    this.missing.forEach(yaml => {
      var _a
      const type =
        ((_a = yaml.info) === null || _a === void 0 ? void 0 : _a.category) ===
        "esoteric"
          ? "esolang"
          : "pl"
      this.createFile(
...
tags ${type}`,
  ...
  mergeOne(match) {
    ...
    if (info) {
      ...
      if (
        ((_a = info.web) === null || _a === void 0 ? void 0 : _a.esolang) &&
        !object.esolang
      )
        ConceptFile.set(
          "esolang",
          (_b = info.web) === null || _b === void 0 ? void 0 : _b.esolang
        )
      if (info.ext)
        node.set(
          "fileExtensions",
