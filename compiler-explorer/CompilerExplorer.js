...
const langPath = path.join(__dirname, "cache", "compiler-explorer", "out", "dist", "lib", "languages.js");
const prepLangFile = () => {
    // todo: path.join(__dirname + '/../../../examples', key, 'default' + lang.extensions[0]),
    const content = Disk.read(langPath)
        .replace(`import path from 'path';
import fs from 'fs-extra';
import _ from 'underscore';`, `const path = require('path');
...
class ConceptFileWithCompilerExplorer {
    ...
    writeInfo() {
        ...
        const { example } = lang;
        if (example)
            compilerExplorerNode.touchNode("example").setChildren(example);
        file.prettifyAndSave();
        return this;
    }
}
class CompilerExplorerImporter extends MeasurementsCrawler_1.MeasurementsCrawler {
    init() {
        prepLangFile();
    }
    get languages() {
    ...
    writeDataCommand() {
        this.matched.forEach(pair => new ConceptFileWithCompilerExplorer(pair.file, pair.lang).writeInfo());
    }
