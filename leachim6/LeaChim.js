...
exports.LeaChimImporter = void 0;
...
class LeaChimImporter extends MeasurementsCrawler_1.MeasurementsCrawler {
    parseAllCommand() {
        const entries = [];
        ...
        entries.forEach(entry => {
            ...
            tree.appendLineAndChildren(entry.file, `name ${entry.name}
filepath ${path}
code
 ${entry.code.replace(/\n/g, "\n ").replace(/\r/g, "")}`);
            if (entry.extension)
    ...
    writeAllCommand() {
        this.matched.forEach(match => {
            ...
            node.appendLineAndChildren("example", hit.getNode("code").childrenToString());
...
exports.LeaChimImporter = LeaChimImporter;
