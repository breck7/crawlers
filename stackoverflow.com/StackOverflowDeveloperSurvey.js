...
class StackOverflowDeveloperSurveyImporter extends MeasurementsCrawler_1.MeasurementsCrawler {
    constructor() {
        super(...arguments);
        this.users = {};
        this.totalRows = 0;
    ...
    writeToDatabaseCommand() {
        ...
        Object.values(objects).forEach((row) => {
            ...
            file.appendLineAndChildren("stackOverflowSurvey", `2021
 users ${row.users}
 ...
 percentageUsing ${row.percentageUsing}`);
    ...
    */
    ...
    processRow(row) {
        const { users } = this;
        const { LanguageHaveWorkedWith, // C++;HTML/CSS;JavaScript;Objective-C;PHP;Swift
        LanguageWantToWorkWith, // C++;HTML/CSS;JavaScript;Objective-C;PHP;Swift
        ConvertedCompYearly } = row;
        this.totalRows++;
