...
class PygmentsImporter extends TreeBaseCrawler_1.TreeBaseCrawler {
    linkAllCommand() {
    ...
    extractData(file, entry) {
        if (!file.has("pygmentsHighlighter"))
            return;
    ...
    writeOne(file, entry) {
        ...
        if (extensions)
            file.set("pygmentsHighlighter fileExtensions", extensions);
        file.prettifyAndSave();
    }
    get data() {
    ...
    get match() {
        return this.data.map(entry => {
            ...
            if (entry.treeBaseId)
                entry.file = this.base.getFile(entry.treeBaseId);
            return entry;
        });
    }
    get matches() {
